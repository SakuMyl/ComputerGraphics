...
void Skeleton::setJointRotation(unsigned index, Vec3f euler_angles) {
	...
	Mat3f xrot = Mat3f::rotation(Vec3f(1, 0, 0), euler_angles[0]);
	Mat3f yrot = Mat3f::rotation(Vec3f(0, 1, 0), euler_angles[1]);
	Mat3f zrot = Mat3f::rotation(Vec3f(0, 0, 1), euler_angles[2]);
	Mat3f rot = xrot * yrot * zrot;
...
void Skeleton::updateToWorldTransforms(unsigned joint_index, const Mat4f& parent_to_world) {
	...
	joint.to_world = parent_to_world * joint.to_parent;
	for (auto child : joint.children) {
		updateToWorldTransforms(child, joint.to_world);
	}
}
